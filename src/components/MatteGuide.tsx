...
import { ArrowLeft, CheckCircle, ChevronRight, ShieldCheck, Info, BookOpen, Clock, Sparkles } from "lucide-react";
...
export function MatteGuide({ onBack }: { onBack: () => void }) {
    ...
    const markComplete = async (id: string) => {
        if (!completedSteps.includes(id)) {
            ...
            try {
                await api.patch('/users/me/course-progress', { courseName: 'MATTE', stepId: id });
            } catch (err) {
    ...
    useEffect(() => {
        const fetchProgress = async () => {
            try {
                ...
                const progress = response.data.courseProgress?.MATTE || [];
                        ...
                        <div className="bg-gray-50 rounded-3xl p-8 sm:p-12 border border-gray-100 relative overflow-hidden group mb-3">
                            ...
                            <div className="relative z-10">
                                <Badge className="bg-[#0EA0DC]/10 text-[#0EA0DC] border-[#0EA0DC]/20 mb-4 px-3 py-1 font-bold">
                                    PROFESSIONAL MATTE APPLICATION GUIDE
                                </Badge>
                                <h1 className="text-4xl sm:text-5xl font-bold text-[#272727] mb-4 tracking-tighter uppercase">
                                    <span className="text-[#0EA0DC]">MATTE</span> Professional Guide
                                </h1>
                                <p className="text-[#666666] text-lg max-w-2xl mb-8 font-medium">
                                    Preserve the original texture and finish of matte surfaces with SkyGloss MATTE — a specialized professional protection system.
                                </p>
                            ...
                            <Card className="p-12 rounded-2xl bg-[#272727] text-white text-center">
                                ...
                                <div className="flex justify-center">
                                    <Button
                                        onClick={async () => {
                                            try {
                                                await api.patch('/users/me/complete-course', { courseName: 'MATTE' });
                                            } catch (err) {
