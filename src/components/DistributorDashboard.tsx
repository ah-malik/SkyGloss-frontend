...
export function DistributorDashboard({
  ...
  const handleAddFromProductPage = (size: string, quantity: number, price?: number, oType?: "unit" | "case") => {
    const product = distributorProducts.find(p => p.id === viewingProduct);
    if (!product) return;

    // Call existing addToOrder with normalized parameters
                          ...
                          <Card className="skygloss-card p-4 sm:p-6 rounded-2xl">
                            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                              ...
                              <div className="flex-1 min-w-0">
                                <h3
                                  className="text-base sm:text-lg text-[#272727] mb-3 sm:mb-4 cursor-pointer hover:text-[#0EA0DC] transition-colors"
                                  onClick={() => setViewingProduct(product.id)}
                                >
                                  {product.name}
                                </h3>
